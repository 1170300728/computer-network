...
int main(int argc, char* argv[])
{
	...
	while (true) {
		...
		if (!strcmp(cmd, "-testgbn")) {
			...
			unsigned short RecvAck;//当前获得的Ack
			...
			while (true)
			{
				...
				case 1://等待接收数据阶段 
					...
					RecvAck = (unsigned short)buffer[0];
					//随机法模拟包是否丢失 
					b = lossInLossRatio(packetLossRatio);
					if (b) {
						printf("The packet with a seq of %d and an Ack of %d loss\n", seq, RecvAck);
						continue;
					}
					printf("recv a packet with a seq of %d and an Ack of %d\n", seq, RecvAck);
					...
					else if (seq >= waitSeq && (waitSeq + RECV_WIND_SIZE > SEQ_SIZE ? true : seq < (waitSeq + RECV_WIND_SIZE))) {//在接收窗口范围内if (!(seq - waitSeq)){ ++waitSeq; if (waitSeq == 21){waitSeq = 1; } //在这里应该向上层交付数据}
