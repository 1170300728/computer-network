...
// Parameter: char c 
//************************************ 
void ackHandler(char c1,char c2) {
	unsigned char index = (unsigned char)c1 - 1; //序列号减一 
			...
			while (runFlag) {
				...
				case 2://数据传输阶段 
					if (seqIsAvailable() && totalSeq - SEND_WIND_SIZE <= totalPacket) {
						//发送给客户端的序列号从 1 开始 
						buffer[0] = curSeq + 1;
						buffer[1] = (RecvAck == curAck ? curAck : -1);
						ack[curSeq] = 0;
						//数据发送的过程中应该判断是否传输完成 
						//为简化过程此处并未实现 
					...
					timeval tv = { 1, 0 };
					setsockopt(sockServer, SOL_SOCKET, SO_RCVTIMEO, (char*)&tv, sizeof(timeval));
